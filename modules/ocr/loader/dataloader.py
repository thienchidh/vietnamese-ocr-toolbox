...
from .storage import OCRStorage
...
class OCRDataset(Dataset):
    def __init__(self, lmdb_path, root_dir, annotation_path, vocab, image_height=32, image_min_width=32,
        ...
        self.init_storage() # for first time

        nSamples = int(OCRStorage.get_instance().txn.get('num-samples'.encode()))
        self.nSamples = nSamples

        self.build_cluster_indices()

    def init_storage(self):
        OCRStorage.get_instance().env = lmdb.open(
            self.lmdb_path,
            max_readers=8,
            readonly=True,
            lock=False,
            readahead=False,
            meminit=False)
        OCRStorage.get_instance().txn = OCRStorage.get_instance().env.begin(write=False)

    def build_cluster_indices(self):
        print('Building cluster indices')
    ...
    def get_bucket(self, idx):
        ...
        dim_img = OCRStorage.get_instance().txn.get(key.encode())
    ...
    def read_buffer(self, idx):
        ...
        imgbuf = OCRStorage.get_instance().txn.get(img_file.encode())

        label = OCRStorage.get_instance().txn.get(label_file.encode()).decode()
        img_path = OCRStorage.get_instance().txn.get(path_file.encode()).decode()
    ...
    def __getitem__(self, idx):
        if OCRStorage.get_instance().txn is None:
            self.init_storage()
